...
app.get("/tweets/:tweetId/", authenticateToken, async (request, response) => {
  ...
  if (
      userFollowers.some(item)=>item.following_user_id===tweetResults.user_id
  )
){
...
app.get("/tweets/:tweetId/likes/", authenticateToken, async (request, response) => {
  ...
  const { user_id, name, username, gender } = payload;
  console.log(name,tweetId);
  const getLikesQuery = `Select * from 
  follower INNER JOIN tweet ON tweet.user_id=follower.following.following_user_id INNER JOIN like ON like.tweet_id=tweet.tweet_id
...
app.get("/tweets/:tweetId/replies/", authenticateToken, async (request, response) => {
  ...
  const { user_id, name, username, gender } = payload;
  console.log(name,tweetId);
  const getRepliesQuery = `Select * from 
  follower INNER JOIN tweet ON tweet.user_id=follower.following.following_user_id INNER JOIN reply ON reply.tweet_id=tweet.tweet_id
  INNER JOIN user ON user.user_id=user.user_id
  WHERE tweet.tweet_Id=${tweetId} AND follower.follower_user_id=${user_id};`;


  const repliedUsers=await db.all(getReliesQuery);
